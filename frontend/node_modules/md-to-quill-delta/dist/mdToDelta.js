...
var MarkdownToQuill = /** @class */ (function () {
    function MarkdownToQuill(options) {
        this.blocks = ['paragraph', 'code', 'heading', 'blockquote', 'list', 'table'];
            ...
            children.forEach(function (child, idx) {
                ...
                switch (child.type) {
                    ...
                    case 'table':
                        ...
                        break;
                    case 'tableRow':
                        delta = delta.concat(_this.convertChildren(node, child, op, indent, __assign(__assign({}, extra), { id: _this.generateId() })));
                        break;
                    case 'tableCell':
                        var align = extra && extra.align;
                        var alignCell = align && Array.isArray(align) && align.length > idx && align[idx];
                        if (_this.options.debug) {
                            console.log('align', alignCell, align, idx);
                        }
                        delta = delta.concat(_this.convertTableCell(node, child, extra && extra.id, alignCell));
                        break;
                    case 'heading':
                        delta = delta.concat(_this.convertChildren(node, child, op, indent + 1));
                        delta.push({
    ...
    MarkdownToQuill.prototype.generateId = function () {
    ...
    MarkdownToQuill.prototype.convertTableCell = function (parent, node, tableId, align) {
        var delta = new Delta();
        delta = delta.concat(this.convertChildren(parent, node, {}, 1));
        var attributes = { table: tableId };
        if (align && align !== 'left') {
